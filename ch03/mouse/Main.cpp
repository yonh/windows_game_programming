...
int mouse_x, mouse_y,btns; // mouse x, mouse y, mouse click buttons
...
switch(msg)
	...
	case WM_MOUSEMOVE:
		...
		if (btns & MK_LBUTTON) {
			OutputDebugString("mouse left key is pressed\n");
		}
		if (btns & MK_RBUTTON) {
			OutputDebugString("mouse right key is pressed\n");
		}

		hdc = BeginPaint(hwnd,&ps);	 
        SetTextColor(hdc, RGB(220,255,0));
		...
		break;
	case WM_RBUTTONDOWN:
		MessageBox(NULL, "right button is pressed", "title",MB_OK);
		break;
	default:break;
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR lpcmdline, int ncmdshow)
{
	...
	while (TRUE) {
		...
		if (KEYDOWN(VK_ESCAPE)) // if pressed key esc quit program
		{
			SendMessage(hwnd, WM_CLOSE, 0, 0);
			// PostMessage(hwnd, WM_CLOSE, 0,0); // also can quit
		}
